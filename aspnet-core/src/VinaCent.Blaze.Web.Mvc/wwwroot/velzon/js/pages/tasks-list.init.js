    ...
    editBtns.forEach(function (btn) {
        btn.addEventListener("click", function (e) {
            ...
            itemValues.forEach(function (x) {
                ...
                var selectedid = isid.body.firstElementChild.innerHTML;
                if (selectedid == itemId) {
                    // var assign = new DOMParser().parseFromString(x._values.assignedto, "text/html");
                    // var allAnchor = assign.querySelectorAll('body .avatar-group a');
                    // allAnchor.forEach(function (a){
                    //     var imgName = a.getAttribute('data-img');
                    //     var assignedTo = document.querySelectorAll('input[name="assignedTo[]"]');
                    //     for(var x = 0, l = assignedTo.length; x < l;  x++){
                    //         if(assignedTo[x].value == imgName){
                    //             assignedTo[x].checked = true;
                    //             break;
                    //         }
                    //     }
                    // });
                    ...
                    //statusField.value = x._values.status;
...
function isStatus(val) {
    switch (val) {
        ...
        case "Completed":
            return (
                '<span class="badge badge-soft-success text-uppercase">' + val + "</span>"
            );
        case "New":
            return (
...
function assignToUsers(){
    ...
    if (assignedTo.length > 0) {
        for(var x = 0, l = assignedTo.length; x < l;  x++){
            assignedtousers += `
                <a href="javascript: void(0);" class="avatar-group-item" data-img="${assignedTo[x].value}" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Username">
                    <img src="assets/images/users/${assignedTo[x].value}" alt="" class="rounded-circle avatar-xxs" />
                </a>
            `;
        }
    } else {
