...
(function () {
    ...
    function setDefaultAttribute() {
        if (!sessionStorage.getItem("defaultAttribute")) {
            ...
            if (abp.session.userId > 0) {
                isLayoutAttributes["data-layout-mode"] = abp.setting.get("App.UiThemeMode");
            }
    ...
    function saveUiModeInLocal(modeType) {
        if (modeType == "dark") {
    ...
    function setLayoutMode(mode, modeType, modeTypeId, html) {
        ...
        // If user logged in
        if (abp.session.userId > 0) {
            abp.services.app.configuration.changeUiThemeMode({
                themeMode: modeType
            });
        }
        saveUiModeInLocal(modeType);
    }
